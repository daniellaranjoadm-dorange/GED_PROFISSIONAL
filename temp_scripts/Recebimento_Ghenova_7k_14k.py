...
BASE_DIRS = [
    {
        "empreendimento": "14K",
        "base_dir": Path(r"\\virm-rgr022\FILESERVER\Departamento\DENG - Engenharia\98 - ESTUDOS\TRANSPETRO GASEIROS\Gaseiro 14k\(001) DOCUMENTOS DE ENGENHARIA"),
        "output_filename": "LD recebidos GHENOVA 14K.xlsx",
    },
    {
        "empreendimento": "7K",
        "base_dir": Path(r"\\virm-rgr022\FILESERVER\Departamento\DENG - Engenharia\98 - ESTUDOS\TRANSPETRO GASEIROS\Gaseiro 7k\(001) DOCUMENTOS DE ENGENHARIA"),
...
def salvar_excel(df: pd.DataFrame, output_path: Path, empreendimento: str):
...
def processar_base(base_dir: Path, empreendimento: str, output_filename: str):
    ...
    df = pd.DataFrame(todos_registros, columns=[
        "Empreendimento",
        "GRD",
        "Subpasta",
        "Pasta",

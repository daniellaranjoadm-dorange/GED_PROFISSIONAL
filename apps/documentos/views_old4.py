...
def enviar_proxima_etapa(request, documento_id):
    ...
    etapa_atual = documento.etapa
    proxima = etapa_atual.proxima_etapa if etapa_atual else None

    if not proxima:
        messages.warning(request, "Este documento já está na última etapa do workflow.")
        return redirect("documentos:detalhes_documento", documento_id=documento.id)

    documento.etapa = proxima
    documento.save(update_fields=["etapa"])
    ...
    DocumentoWorkflowHistorico.objects.create(
        ...
        acao="AVANÇAR",
        observacao="Avanço manual via interface"
    )

    messages.success(request, f"Documento avançado para: {proxima.nome}")
    return redirect("documentos:detalhes_documento", documento_id=documento.id)


    # 🔥 SINCRONIZA CAMPO LEGADO (MUITO IMPORTANTE)
    if hasattr(documento, "etapa_atual"):
        documento.etapa_atual = nova_etapa.codigo
        documento.save(update_fields=["etapa_atual"])

    messages.success(
        request,
        f"Documento retornado para a etapa: {nova_etapa.nome}"
    )
...
def retornar_etapa(request, documento_id):
    ...
    if not motivo:
        messages.warning(request, "Informe o motivo do retorno da etapa.")
        return redirect("documentos:detalhes_documento", documento_id=documento.id)

    etapa_atual = documento.etapa
    etapa_anterior = etapa_atual.etapa_anterior if etapa_atual else None

    if not etapa_anterior:
        messages.warning(request, "Documento já está na primeira etapa.")
        return redirect("documentos:detalhes_documento", documento_id=documento.id)

    documento.etapa = etapa_anterior
    documento.save(update_fields=["etapa"])
    ...
    # 🔹 Anexos no retorno
    for f in arquivos:
        DocumentoWorkflowAnexo.objects.create(
            historico=historico,
            arquivo=f,
    ...
    messages.success(
        request,
        f"Documento retornado para: {etapa_anterior.nome}"
    )
    return redirect("documentos:detalhes_documento", documento_id=documento.id)

...
def enviar_proxima_etapa(request, documento_id):
    ...
    if not pode_avancar_etapa(request.user, documento):
        messages.error(request, "Você não tem permissão para avançar este documento.")
        return redirect("documentos:detalhes_documento", documento_id=documento.id)
...
# 🔥 GARANTIA: sincroniza legado se ainda existir
...
documento.save(update_fields=["etapa_atual"])
    ...
    if not nova_etapa:
        messages.warning(
            request,
            "Este documento já está na última etapa do workflow."
        )
        return redirect("documentos:detalhes_documento", documento_id=documento.id)

    messages.success(
        request,
        f"Documento avançado de "
        f"{etapa_anterior.nome if etapa_anterior else 'Início'} "
        f"→ {nova_etapa.nome}"
    )
...
# =================================================================
# RETORNAR ETAPA — WORKFLOW ENTERPRISE
# =================================================================
...
def retornar_etapa(request, documento_id):
    ...
    nova_etapa = documento.retornar_etapa(
        etapa_destino=etapa_prev,
        usuario=request.user,
        motivo=motivo
    )

    if not nova_etapa:
        messages.error(request, "Falha ao retornar etapa.")
        return redirect("documentos:detalhes_documento", documento_id=documento.id)

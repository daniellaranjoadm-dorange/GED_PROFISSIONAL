...
PASTA_PDFS = Path(
    r"\\virm-rgr022\FILESERVER\Projetos\05_HANDYMAX\09. Doc Control\15 - Documentos KM\4 KM Equipment"
)

ARQUIVO_EXCEL_NOVO = Path(
    r"\\virm-rgr022\FILESERVER\Projetos\05_HANDYMAX\09. Doc Control\15 - Documentos KM\4 KM Equipment\Lista Equipamentos KM.xlsx"
)
...
def extrair_transmittal(texto: str, nome_arquivo: str) -> str:
    m = re.search(r"TRANSMITTAL\s+No\.?:\s*([A-Z0-9-]+)", texto, re.IGNORECASE)
    if m:
    ...
    m = re.search(r"(\d{7}-TR-\d{5})", nome_arquivo, re.IGNORECASE)
    if m:
        return m.group(1).strip().upper()
...
def extrair_data_envio(texto: str) -> str:
    m = re.search(r"Date:\s*(\d{4}-\d{2}-\d{2})", texto, re.IGNORECASE)
    if m:
...
def extrair_transmittal_equipment(texto: str, nome_arquivo: str) -> str:
    """
    Extrai número de transmittal do layout KM Equipment.

    Exemplo:
      TRANSMITTAL No.: 6836907-TR-00002
    """
    m = re.search(r"TRANSMITTAL\s+No\.?:\s*([A-Z0-9-]+)", texto, re.IGNORECASE)
    if m:
        return limpar_valor(m.group(1)).upper()

    m = re.search(r"([0-9]{7}-TR-[0-9]{5})", nome_arquivo, re.IGNORECASE)
    if m:
        return limpar_valor(m.group(1)).upper()

    return extrair_transmittal(texto, nome_arquivo)


def extrair_data_envio_equipment(texto: str) -> str:
    """
    Extrai data do layout KM Equipment.

    Exemplo:
      Date: 2026-04-30
    """
    m = re.search(r"\bDate:\s*(\d{4}-\d{2}-\d{2})\b", texto, re.IGNORECASE)
    if m:
        return limpar_valor(m.group(1))

    return extrair_data_envio(texto)


def _eh_linha_documento_equipment(linha: str):
    """
    Identifica linhas do bloco Included Documents.

    Formato observado:
      5458675 A IFA System Drawing - Alarm and
      5461222 A IFI Technical Manual - Installation
    """
    return re.match(
        r"^\s*(\d{6,8})\s+([A-Z0-9]+)\s+([A-Z]{2,5})\s+(.+?)\s*$",
        linha,
        re.IGNORECASE,
    )


def extrair_registros_equipment_included_documents(texto: str, nome_arquivo: str, caminho_pdf: Path) -> List[Dict[str, str]]:
    """
    Parser específico para transmittals KM Equipment com seção:
      Included Documents
      Doc No Revision Reason for Issue Title Comments

    Mapeamento:
      Documento             = Doc No
      Emissão               = Revision
      Proposito de Emissão  = Reason for Issue
      Titulo                = Title + continuações
      Pasta                 = vazio, pois o PDF não traz pasta documental explícita
    """
    texto = normalizar_texto(texto)

    if "Included Documents" not in texto:
        return []

    transmittal = extrair_transmittal_equipment(texto, nome_arquivo)
    data_envio = extrair_data_envio_equipment(texto)
    ...
    for linha in linhas:
        ...
        if any(linha.startswith(prefixo) for prefixo in ignorar_prefixos):
            continue

        if re.match(r"^\d+\s*/\s*\d+$", linha):
            continue

        if re.search(r"^Number of documents:\s*\d+", linha, re.IGNORECASE):
            break

        m = _eh_linha_documento_equipment(linha)
        if m:
            if atual:
                registros.append(atual)

            documento, revisao, reason, titulo = m.groups()
            atual = {
                "Documento": limpar_valor(documento),
                "Titulo": limpar_valor(titulo),
                "Pasta": "",
                "Emissão": limpar_valor(revisao),
                "Proposito de Emissão": limpar_valor(reason),
        ...
        # Continuação do título da linha anterior
        if atual:
            atual["Titulo"] = limpar_valor(f"{atual.get('Titulo', '')} {linha}")
...
def extrair_registros_km_equipment(texto: str, nome_arquivo: str, caminho_pdf: Path) -> List[Dict[str, str]]:
    """
    Parser específico para PDFs KM Equipment.
    Captura listas em múltiplas páginas até "Number of documents:".
    """
    texto = normalizar_texto(texto)
    ...
    padrao_doc = re.compile(
        r"^([A-Z]{0,3}\d[A-Z0-9-]*)\s+([A-Z][A-Z0-9]*)\s+(IFI|IFA|IFC|AS[- ]?BUILT)\s+(.+)$",
        re.IGNORECASE,
    )

    ignorar_inicio = re.compile(
        r"^(TRANSMITTAL|PO No|Hull|Project|Remarks|Distribution|Name|Company|Included Documents|"
        r"Doc No|Project Doc No|Revision|Reason for|Issue|Title|Comments|Planned|Return|Date:|"
        r"\d+\s*/\s*\d+)$",
        re.IGNORECASE,
    )
    ...
    for linha in linhas:
        if re.search(r"Included Documents", linha, re.IGNORECASE):
            lendo_tabela = True
            continue
        ...
        if not lendo_tabela:
            if not padrao_doc.match(linha):
                continue
            lendo_tabela = True

        if ignorar_inicio.match(linha):
            continue

        match_doc = padrao_doc.match(linha)
        if match_doc:
            if atual:
                registros.append(atual)

            atual = {
                "Documento": limpar_valor(match_doc.group(1)),
                "Titulo": limpar_valor(match_doc.group(4)),
                "Pasta": "",
                "Emissão": limpar_valor(match_doc.group(2)),
                "Proposito de Emissão": limpar_valor(match_doc.group(3)),
                "Data Envio": data_envio,
                "Transmittal N°": transmittal,
                "Arquivo PDF": str(caminho_pdf),
                "Status Parse": "OK",
        ...
        if atual:
            if ignorar_inicio.match(linha):
                continue
            if linha.upper() in {"IFI", "IFA", "IFC", "AS-BUILT"}:
                continue
            atual["Titulo"] = limpar_valor(f"{atual.get('Titulo', '')} {linha}")
    ...
    for item in registros:
        chave = (
            item.get("Documento", "").upper(),
            item.get("Emissão", "").upper(),
            item.get("Transmittal N°", "").upper(),
        )
        if chave in vistos:
            continue
        vistos.add(chave)
...
def extrair_registros_pdf(texto: str, nome_arquivo: str, caminho_pdf: Path) -> List[Dict[str, str]]:
    registros_equipment = extrair_registros_km_equipment(texto, nome_arquivo, caminho_pdf)
    if registros_equipment:
        return registros_equipment

    texto = normalizar_texto(texto)

    # 1) Parser específico para layout KM Equipment:
    #    TRANSMITTAL No. + Included Documents
    registros_equipment = extrair_registros_equipment_included_documents(texto, nome_arquivo, caminho_pdf)
    if registros_equipment:
        return registros_equipment

    # 2) Parser legado de Transmittal Letters:
...
def executar():
    ...
    try:
        ...
        if not isinstance(resumo, dict):
            return {
                "ok": False,
                "mensagem": "KM Equipment não retornou resumo de processamento.",
        ...
        if not ok:
            return {
                "ok": False,
                "mensagem": resumo.get("mensagem") or "KM Equipment não foi concluído.",
        ...
        return {
            "ok": True,
            "mensagem": (
                "KM Equipment executado com sucesso. "
        ...
        return {
            "ok": False,
            "mensagem": f"Erro no KM Equipment: {e}",

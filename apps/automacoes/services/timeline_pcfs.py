...
def aplicar_estilo_tabela(ws, headers):
    ...
    for c, h in enumerate(headers, start=1):
        ...
        ws.column_dimensions[col_letter].width = widths.get(
            h, max(14, min(40, len(safe_str(h)) + 4))
        )
...
def salvar_pcf_no_banco(tipo, row):
    numero_documento = safe_str(row.get("Nº DOCUMENTO"))
    revisao = safe_str(row.get("Revisão da PCF"))
    pcf_link = safe_str(row.get("PCF LINK"))

    if not numero_documento:
        return False

    PCFTimeline.objects.update_or_create(
        tipo=safe_str(tipo),
        numero_documento=numero_documento,
        revisao_pcf=revisao,
        pcf_link=pcf_link,
        defaults={
            "caminho": safe_str(row.get("Caminho")),
            "numero_pcf": safe_str(row.get("N º PCF")),
            "titulo": safe_str(row.get("TITULO")),
            "data_recebimento": safe_str(row.get("Data Recebimento")),
            "open_comments": _safe_int(row.get("Open Comments")),
            "qtd_comentarios": _safe_int(row.get("Qtd Comentarios")),
            "status_final": safe_str(row.get("STATUS FINAL")),
        },
    )
    return True
    ...
    for aba, pasta in PASTAS_PCF.items():
        ...
        if len(arquivos) == 0:
            ...
            resumo[aba] = {
                "arquivos": 0,
                "linhas": 0,
                "erros": ["0 arquivos encontrados; aba preservada."],
            }
            continue
        ...
        for idx, caminho in enumerate(arquivos, start=1):
            ...
            try:
                ...
                if row is None:
                    ignorados_modelo += 1
                    msg = f"Ignorada como nao-PCF/modelo invalido: {caminho}"
                    if MOSTRAR_CADA_PCF_IGNORADA:
                        log_aviso(msg)
                    continue

                rows.append(row)
                salvar_pcf_no_banco(aba, row)
        ...
        if len(rows) == 0:
            log_aviso(
                f"{len(arquivos)} arquivos encontrados, mas 0 PCFs lidas em '{aba}'. Aba preservada."
            )
            if erros:
                log("Primeiros erros:")
            ...
            resumo[aba] = {
                ...
                "linhas": 0,
                "erros": erros or ["0 arquivos lidos; aba preservada."],
            }
            continue
...
def build_headers(order_list):
    return [
        ...
        "De",
        "Para",
    ] + order_list[:] + [f"Δ {order_list[i]}-{order_list[i-1]}" for i in range(1, len(order_list))] + [
        "Δ Total"
    ]
...
def recriar_evolucao(wb):
    ...
    ws_rec_src = (
        wb[SHEET_SRC_RECEBIDAS] if SHEET_SRC_RECEBIDAS in wb.sheetnames else wb.create_sheet(SHEET_SRC_RECEBIDAS)
    )
    ws_res_src = (
        wb[SHEET_SRC_RESPONDIDAS]
        if SHEET_SRC_RESPONDIDAS in wb.sheetnames
        else wb.create_sheet(SHEET_SRC_RESPONDIDAS)
    )

    rows_rec = compute_evolution_rows(build_map_from_source(ws_rec_src, ORDER_RECEBIDAS), ORDER_RECEBIDAS)
    rows_res = compute_evolution_rows(
        build_map_from_source(ws_res_src, ORDER_RESPONDIDAS), ORDER_RESPONDIDAS
    )
...
def salvar_com_trava(wb, resumo, backup_path):
    ...
    fd, temp_path = tempfile.mkstemp(
        prefix="Timeline_PCFs_TEMP_", suffix=".xlsx", dir=pasta_destino
    )
    os.close(fd)
...
def executar():
    try:
        ...
        for aba, info in resumo.items():
            log(
                f"{aba}: {info.get('linhas', 0)} linhas gravadas de "
                f"{info.get('arquivos', 0)} arquivos candidatos."
            )
